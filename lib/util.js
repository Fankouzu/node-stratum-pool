...
exports.addressToScript = function(addr){
    try {
        // This will handle all types of addresses: legacy, SegWit, and Taproot
        const { version, hash } = bitcoin.address.fromBech32(addr);
        if (version === 1 && hash.length === 32) {
            // Taproot address
            return Buffer.concat([Buffer.from([0x51, 0x20]), hash]);
        } else {
            // SegWit address
            return bitcoin.script.compile([version, hash]);
        }
    } catch (e) {
        // If it's not a Bech32 address, try Base58
        try {
