...
//     var decoded = Buffer.from(addr).toString('hex');
...
// };
exports.addressToScript = function(addr){
    // Check if it's a Taproot address (starts with bc1p for mainnet or tb1p for testnet)
    if (addr.startsWith('bc1p') || addr.startsWith('tb1p')) {
        try {
            const { prefix, words } = bech32.decode(addr);
            const data = bech32.fromWords(words);
            if (data.length !== 32) {
                console.error('invalid taproot address length for ' + addr);
                throw new Error('Invalid Taproot address');
            }
            return Buffer.concat([Buffer.from([0x51, 0x20]), Buffer.from(data)]);
        } catch (error) {
            console.error('Error decoding Taproot address:', error);
            throw error;
        }
    }
    
    // For legacy or SegWit addresses (keep the existing code)
    var decoded = base58.decode(addr);
    ...
    return Buffer.concat([
        util.uint8BufferFromHex(version),
        Buffer.from([0x14]),
        pubkey
    ]);
};
...
exports.bignumFromBitsBuffer = function(bitsBuff){
    ...
    var target = bigBits * (BigInt(2) ** BigInt(8 * (numBytes - 3)));
    return target;
};
